/**
 * Pyton-get-result.js
 * Full state machine — LOGIN → HOME → SETUP → TREND → ANALYZING → RESULT
 * Works with antimaster.php to keep card always visible
 */
var T = (function () {
  'use strict';
  ...
  var S = { LOGIN:'login', HOME:'home', SETUP:'setup',
            TREND:'trend', ANA:'analyzing', RESULT:'result' };
  ...
  function init() {
    ...
    /* Toggle collapse/expand on ▼ button */
    if (toggle) {
    ...
    /* Also expand on header click when collapsed */
    var hdr = $('tcHeader');
    if (hdr) {
    ...
    go(S.LOGIN);
    setTimeout(function () { setChip('ok', 'Server Connected.. Login Now'); }, 900);
  }

  /* ══════ CHIP STATUS ══════ */
  function setChip(type, text) {
    if (dot)     dot.className     = 'chip-dot ' + type;
    if (chipTxt) chipTxt.textContent = text;
  }
  ...
  function go(s) {
    ...
    /* Always make sure card is expanded when state changes */
    if (card) {
      card.classList.remove('collapsed');
      if (toggle) toggle.textContent = '▼';
    }
  ...
  function runSetup() {
    ...
    var iv   = setInterval(function () {
      ...
      if (i < msgs.length) {
        ...
        clearInterval(iv);
        setTimeout(function () { go(S.TREND); }, 350);
      }
    }, 650);
  }

  /* ══════ NAV ACTIONS (called from HTML onclick) ══════ */
  ...
  function buildSlots() {
    ...
    for (var i = 1; i <= TOTAL; i++) {
      (function (idx) {
        ...
        btn.id        = 'slot' + idx;
        btn.innerHTML = '<span class="sn">T' + idx + '</span>' +
                        '<span class="sv" id="sv' + idx + '">·</span>';
  ...
  function pick(size) {
    ...
    if (bd) {
      bd.textContent = size === 'Big'
        ? '🔵 Big (MB) selected — tap a Trend slot below'
        : '🔴 Small (MS) selected — tap a Trend slot below';
  ...
  function onSlot(idx) {
    ...
    if (trends.length === TOTAL) {
      setTimeout(function () { go(S.ANA); }, 420);
    }
  }

  /* ══════ FLASH BUTTONS (no pick) ══════ */
  function flashBtns() {
  ...
  function buildPayload() {
    var base = 504312610;
    return {
      trends: trends.map(function (t, i) {
        return {
  ...
  function fallback() {
    ...
    var pred = streak >= 3 ? (last === 'Big' ? 'Small' : 'Big')
                           : (bigCnt >= smlCnt ? 'Big' : 'Small');
    ...
    var conf = Math.round(sc * 0.45 + 72 * 0.35 + 68 * 0.20);
  ...
  function render(data) {
    ...
    /* Size card — dynamic colour */
    var isBig  = (p.predicted_size || '').toUpperCase() === 'MB';
    var rcSize = $('rcSize');
    if (rcSize) {
      rcSize.style.background  = isBig ? 'linear-gradient(135deg,#1e3a8a,#1d4ed8)'
                                       : 'linear-gradient(135deg,#7f1d1d,#dc2626)';
      rcSize.style.borderColor = isBig ? '#3b82f6' : '#f87171';
      rcSize.style.boxShadow   = isBig ? '0 4px 16px rgba(29,78,216,.35)'
                                       : '0 4px 16px rgba(220,38,38,.35)';
    }
    set('rSize', p.predicted_size   || '—');
    set('rNum',  p.predicted_number !== undefined ? p.predicted_number : '—');

    /* Color card value — coloured text */
    var colEl = $('rCol');
    if (colEl) {
    ...
    if (fill) {
      fill.style.width = '0%';
      setTimeout(function () { fill.style.width = conf + '%'; }, 80);
    }
  ...
  /* ══════ PUBLIC API ══════ */
  return { goLogin:goLogin, goHome:goHome, goWingo:goWingo, pick:pick, retry:retry };
